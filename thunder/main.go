...
func (s *server) schema() *graphql.Schema {
	...
	healthQuery := builder.Query()
	healthQuery.FieldFunc("health", func() string {
		return "ok"
	})
	...
	metaQuery := builder.Object("Meta", Meta{})
	metaQuery.FieldFunc("randomString", func() string {
		return fmt.Sprintf("random string: %s", time.Now().String())
...
func main() {
	...
	server := &server{
		posts: []post{
			{Title: "first post!", Body: "I was here first!", CreatedAt: time.Now(), Meta: Meta{CreatedAt: "testing", RandomString: "randomString"}},
